...
from video_engine.downloader import VideoDownloader
...
def fetch_transcript(video_id_or_url: str) -> list[dict]:
    ...
    """
    # Build a canonical URL so the hybrid engine can always extract the video ID
...
def process_video(youtube_url: str, force_reprocess: bool = False) -> dict:
    ...
    downloader = VideoDownloader(output_dir=os.path.join(os.path.dirname(__file__), "uploads"))
    ...
    transcript = fetch_transcript(youtube_url)   # fetch early for Speaking detection
    action_results = []
