...
def _find_ffmpeg() -> str:
    ...
    2. 'ffmpeg' on the system PATH (standard installs).
    3. Common Windows extraction directories created by gyan.dev / BtbN builds.
    ...
    # 3. Scan well-known Windows locations (Downloads, Program Files, C:\ffmpeg)
    search_roots = [
        Path.home() / "Downloads",
        Path("C:/ffmpeg"),
    ...
    for root in search_roots:
        ...
        for candidate in root.rglob("ffmpeg.exe"):
            if candidate.is_file():
                logger.info(f"[transcript_engine] Located ffmpeg at: {candidate}")
                return str(candidate)

    raise RuntimeError(
        "ffmpeg binary not found.\n"
        "  • Windows : Extract ffmpeg and add its 'bin' folder to your PATH\n"
...
def extract_audio(video_url: str) -> str:
    ...
    ydl_opts = {
        ...
        "quiet":       True,
        "noplaylist":  True,
        # Do NOT use postprocessor to avoid needing system ffmpeg at download time;
        # we call our own _convert_to_wav() below.
    }
    with YoutubeDL(ydl_opts) as ydl:
        ydl.download([video_url])

    # Find whatever file yt_dlp wrote
    raw_file = None
...
def _get_audio_duration(wav_path: str) -> float:
    ...
    # Fallback: Python's wave module (only works for PCM WAV)
    try:
...
def fallback_whisper_transcription(video_url: str) -> list[dict]:
    ...
    • Automatically detects non-English speech (does NOT force English).
...
def get_transcript(video_url: str) -> list[dict]:
    ...
    Args:
        video_url: Any valid YouTube URL
                   (e.g. https://www.youtube.com/watch?v=…, https://youtu.be/…).
