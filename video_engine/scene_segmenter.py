"""
video_engine/scene_segmenter.py
...
# ── Scene dataclass (public contract — do not change) ─────────────────────────
...
def detect_scenes(
    ...
    Args:
        ...
        threshold:  ContentDetector HSV-weighted threshold.
                    Lower = more sensitive (use ~20 for high-motion videos,
                    ~27 for standard content).

    Returns:
        List of {"scene_id": int, "start": float, "end": float} dicts.

    Raises:
        RuntimeError: If PySceneDetect is not installed.
        RuntimeError: If PySceneDetect raises any exception during detection.
    """
    ...
    try:
        ...
        logger.info(
            f"[scene_segmenter] PySceneDetect (new API, threshold={threshold}) "
        ...
        # Modern API not available — fall through to legacy attempt
        pass

    except AttributeError:
        # open_video missing — fall through to legacy attempt
        pass

    except Exception as exc:
        # Modern API present but detection crashed — hard failure
        logger.error(
            f"[scene_segmenter] PySceneDetect (new API) raised during detection: {exc}"
        )
        raise RuntimeError(
    ...
    try:
        ...
        logger.info(
            f"[scene_segmenter] PySceneDetect (legacy API, threshold={threshold}) "
            f"→ {len(scene_list)} scenes"
        )
        return [
            {
        ...
        # Neither API installed — hard failure
        logger.error(
            "[scene_segmenter] PySceneDetect is not installed. "
        ...
        # Legacy API present but detection crashed — hard failure
        logger.error(
            f"[scene_segmenter] PySceneDetect (legacy API) raised during detection: {exc}"
        )
        raise RuntimeError(
...
def segment_scenes(
    ...
    Raises:
        RuntimeError: If video_path is not provided.
        RuntimeError: If PySceneDetect is not installed.
        RuntimeError: If PySceneDetect raises during detection.
    """
    if not frames:
        return []

    # ── Hard guard: video_path is mandatory ──────────────────────────────────
    if not video_path:
        logger.error("[scene_segmenter] video_path is required but was not provided.")
        raise RuntimeError(
    ...
    logger.info(
        f"[scene_segmenter] Motion magnitude={motion:.2f} → "
        f"ContentDetector threshold={psd_threshold}"
    )

    # ── Run PySceneDetect — all exceptions propagate as hard failures ─────────
    scene_dicts = detect_scenes(video_path, threshold=psd_threshold)

    # Edge case: no cuts detected → entire video is one scene
    if not scene_dicts:
        logger.warning(
            "[scene_segmenter] No cuts detected — treating entire video as one scene."
        )
    ...
    logger.info(f"[scene_segmenter] Built {len(scenes)} scenes via PySceneDetect.")
    return scenes
...
def legacy_histogram_segmentation(
    ...
    Do NOT wire this into any production code path.
    """
    from .frame_extractor import compute_histogram
    ...
    logger.info(f"[scene_segmenter] legacy_histogram_segmentation → {len(scenes)} scenes.")
    return scenes


# ── Timestamp → scene lookup (public, unchanged) ──────────────────────────────
...
def assign_scene_to_timestamp(scenes: list[Scene], timestamp: float) -> int:
    ...
    # Nearest-boundary fallback (handles floating-point edge cases)
    if scenes:
